 ...
 */
@Config
public class RawIMUMecanumDrivetrain extends MecanumDrive {
    ...
    public RawIMUMecanumDrivetrain(HardwareMap hardwareMap) {
        ...
        imu.resetYaw();
        imu.initialize(new IMU.Parameters(new RevHubOrientationOnRobot(LOGO_FACING_DIR, USB_FACING_DIR)));
        readIMU();
        setCurrentHeading(0);
    ...
    private double headingOffset, heading, angularVelo;
    ...
    public void setCurrentHeading(double angle) {
        headingOffset = normalizeRadians(heading - angle);
    }

    public double getHeading() {
        return normalizeRadians(heading - headingOffset);
    }
